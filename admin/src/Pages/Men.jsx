...
const Men = () => {
    const {menProducts}=useContext(ShoperContext);

    const [searchCategory , setSearchCategory]=useState('');
 
  const allproductlength=menProducts.length;
...
const [men , setmen]=useState('men');
  ...
  const countByCategory = (categoryName) => {
    return menProducts.filter((product) => product.category === categoryName).length;
  };
   ...
   const filterCategory=()=>{
     ...
     if(searchCategory){
      ...
      return   menProducts.filter((product)=>product.category===searchCategory);
     }
...
useEffect(() =>{
  ...
  return (
    ...
    <div className="gender">
      ...
      <Link className={men ? 'homeLink border':'homeLink'}  to="/men">Men</Link>
