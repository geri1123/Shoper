...
const Women = () => {
    const {womenProducts}=useContext(ShoperContext);
    const [women , setwomen]=useState('women');

    const [searchCategory , setSearchCategory]=useState('');
 
  const allproductlength=womenProducts.length;
...
const [selectedProduct, setSelectedProduct] = useState(null);
  ...
  const countByCategory = (categoryName) => {
    return womenProducts.filter((product) => product.category === categoryName).length;
  };
   ...
   const filterCategory=()=>{
     ...
     if(searchCategory){
      ...
      return   womenProducts.filter((product)=>product.category===searchCategory);
     }
...
useEffect(() =>{
  ...
  return (
    ...
    <div className="gender">
        <Link className='homeLink'  to="/">All gender</Link>
      <Link className='homeLink'  to="/men">Men</Link>
      <Link className={women ?'homeLink border' :'homeLink'}  to="/women"> Women</Link>
    </div>
